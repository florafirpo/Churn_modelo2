...
def etapa_testing(df_train, df_test1, df_test2, feature_cols, exp_path):
    ...
    for idx_sem, semilla in enumerate(semillas):
        # Mostrar progreso cada 5 semillas
        if (idx_sem + 1) % 5 == 0:
            logger.info(f"  Progreso: {idx_sem + 1}/{SEMILLAS_EXPERIMENTO} semillas completadas...")
...
def etapa_final(df_train, df_final, feature_cols, exp_path):
    ...
    for idx, semilla in enumerate(semillas, 1):
        ...
        if idx % 10 == 0:
            logger.info(f"  Progreso: {idx}/{SEMILLAS_FINAL} modelos completados...")
...
def generar_submissions(predicciones, exp_path, cortes=None):
    """
    ...
    logger.info(f"Generando {len(cortes)} submissions...")
    ...
    resultados_path = os.path.join(exp_path, "resultados_cortes.csv")
    df_resultados.to_csv(resultados_path, index=False)
    
    logger.info(f"\nArchivos generados:")
    logger.info(f"  ✓ {len(cortes)} submissions en: {kaggle_dir}")
...
def main():
    ...
    print(f"Inicio: {inicio_ejecucion.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    logger.info("="*80)
    logger.info("R_to_py: Workflow Completo de 3 Etapas")
    logger.info("="*80)
