...
- Data drift por mes (202108 vs todos los anteriores)
- Tendencias de bajas
- Variables problemáticas para eliminar (missings altos, baja varianza, drift extremo)
...
# Configuración inicial para evitar warnings y mejorar gráficos
...
# ⚠️ Cambia esta ruta a la ubicación real de tu archivo de datos
DATASET_PATH = "~/datasets/competencia_02_crudo.csv.gz" 
OUTPUT_DIR = "./eda_temporal"

# Mes de interés para la predicción y el análisis de drift
MES_FINAL = 202108  

# Umbrales para detección de problemas
UMBRAL_MISSING_VARIABLE = 0.95  # Si >95% missing en algún mes, marcar
...
def calcular_clase_ternaria(df):
    ...
    # Convierte foto_mes a una unidad de tiempo continua (ej. años*12 + mes)
    df['periodo0'] = (df['foto_mes'] // 100) * 12 + (df['foto_mes'] % 100)
    df = df.sort_values(['numero_de_cliente', 'periodo0']).reset_index(drop=True)
    
    # Calcula el siguiente y subsiguiente periodo del cliente
...
def analizar_registros_por_mes(df):
    ...
    # Mostrar resumen (usando string de pandas para evitar truncado)
    ...
    # Graficar (código de graficación omitido por brevedad, pero mantenido en el script original)
    # ... (Tu código de graficación se mantiene aquí) ...
...
def analizar_missings_por_mes(df):
    ...
    # Graficar evolución de top variables con missings (código de graficación omitido)
    # ... (Tu código de graficación se mantiene aquí) ...
...
def calcular_psi(esperado, actual, bins=10):
    ...
    # Calcular percentiles en datos esperados (Baseline)
    try:
        ...
        # Calcular porcentajes y evitar divisiones/logaritmos de cero
        esperado_pct = esperado_counts / len(esperado_clean)
        actual_pct = actual_counts / len(actual_clean)
        
        # Reemplazar 0 por un valor pequeño para el log (0.0001)
...
def analizar_data_drift(df):
    """
    Analiza data drift comparando MES_FINAL (202108) vs TODOS los meses anteriores (BASELINE).
    """
    print("\n" + "="*80)
    print("ANÁLISIS 3: DATA DRIFT (Population Stability Index)")
    print("="*80)
    
    mes_actual = MES_FINAL # 202108
    meses_anteriores = sorted(df['foto_mes'].unique())
    # Definir el baseline: todos los meses estrictamente anteriores a MES_FINAL
    meses_baseline = [mes for mes in meses_anteriores if mes < mes_actual]
    
    if not meses_baseline:
        print(f"ERROR: No hay meses anteriores a {mes_actual} para el baseline.")
        return pd.DataFrame(), pd.DataFrame()

    print(f"\nComparando mes {mes_actual} (actual) vs BASELINE (Meses: {meses_baseline[0]} a {meses_baseline[-1]})")
    print(f"Umbral PSI: {UMBRAL_DRIFT_PSI} (>0.25 = drift significativo)")
    
    # Filtrar datos
    # BASELINE: Incluye todos los meses anteriores
    df_ref = df[df['foto_mes'].isin(meses_baseline)]
    # ACTUAL: Sólo el mes de interés
    df_ago = df[df['foto_mes'] == mes_actual]
    ...
    # ... (El código de impresión de resultados y gráficos se mantiene) ...
    print(f"\n{'='*80}")
    ...
    if len(vars_drift_alto) > 0:
        ...
        print(f"\n✓ Lista guardada: variables_con_drift_alto.csv")
    else:
    ...
    fig.suptitle(f'Data Drift: Mes {MES_FINAL} vs BASELINE', fontsize=16, fontweight='bold')
...
def analizar_evolucion_variables(df, n_vars=10):
    ...
    variances = []
    for col in cols_analizar:
        # Calcular varianza en todo el dataset para un ranking general
        var = df[col].var() 
    ...
    print(f"\nTop {n_vars} variables por varianza (más informativas):")
    ...
    # Graficar evolución (código de graficación omitido)
    # ... (Tu código de graficación se mantiene aquí) ...
    n_cols = 2
...
def analizar_variables_constantes(df):
    ...
    for var in cols_analizar:
        ...
        # Si la varianza fue baja en TODOS los meses donde la variable existe
        if (var_data['varianza'] < UMBRAL_VARIANZA_CERO).all(): 
            vars_siempre_constantes.append(var)
    
    print(f"\nVariables con varianza < {UMBRAL_VARIANZA_CERO} en mes {MES_FINAL}: Total={len(vars_baja_varianza)}")
    print(f"\nVariables SIEMPRE constantes (en todos los meses): Total={len(vars_siempre_constantes)}")
    
    if len(vars_baja_varianza) > 0:
        vars_baja_varianza[['variable', 'varianza', 'n_unique']].head(20).to_csv(
             os.path.join(OUTPUT_DIR, 'variables_baja_varianza.csv'), index=False
        )
        print("✓ Lista guardada: variables_baja_varianza.csv")
    ...
    if len(vars_siempre_constantes) > 0:
        ...
        print("✓ Lista guardada: variables_siempre_constantes.csv")
...
def generar_reporte_final(clases_por_mes, vars_problematicas_missing,  
    ...
    if not agosto.empty:
        ...
        bajas_agosto = agosto['pct_total_bajas'].values[0]
        reporte.append(f"  - Total registros: {agosto['total'].values[0]:,}")
        reporte.append(f"  - Total Bajas: {bajas_agosto:.2f}%")
    ...
    if len(vars_problematicas_missing) > 0:
        reporte.append(f"\n⚠️  {len(vars_problematicas_missing)} variables con >{UMBRAL_MISSING_VARIABLE*100}% missings en algún mes")
        reporte.append("\nRECOMENDACIÓN: Considerar **ELIMINAR** estas variables (ver 'variables_con_muchos_missings.csv').")
    else:
        reporte.append("\n✓ No hay variables con missings extremos")
    
    # Sección 3: Data Drift
    reporte.append("\n" + "="*80)
    reporte.append("3. VARIABLES CON DATA DRIFT ALTO (PSI)")
    reporte.append("="*80)
    
    # Asegúrate de que vars_drift_alto es un DataFrame o maneja la lista correctamente
    if isinstance(vars_drift_alto, pd.DataFrame):
        vars_drift_alto_df = vars_drift_alto
    else:
        # Esto maneja el caso donde podría ser la lista de nombres de variables si se llamó mal
        vars_drift_alto_df = pd.DataFrame({'variable': vars_drift_alto, 'psi': [np.nan]*len(vars_drift_alto), 'pct_change': [np.nan]*len(vars_drift_alto)})
    
    if len(vars_drift_alto_df) > 0:
        reporte.append(f"\n⚠️  {len(vars_drift_alto_df)} variables con drift significativo (PSI > {UMBRAL_DRIFT_PSI})")
        reporte.append(f"Comparación: Mes {MES_FINAL} vs Distribución Histórica (BASELINE).")
        reporte.append("\nTop 5 Variables con Mayor Drift:")
        for _, row in vars_drift_alto_df.head(5).iterrows():
            reporte.append(f"  - {row['variable']}: PSI={row['psi']:.3f}, Cambio Media={row['pct_change']:.1f}%")
        
        reporte.append("\nRECOMENDACIÓN:")
        reporte.append("  1. **MONITOREAR** estas variables de cerca.")
        reporte.append("  2. Considerar **RE-ENTRENAR** el modelo o usar una ventana temporal más corta si el drift no es estacional.")
    else:
    ...
    if len(vars_siempre_constantes) > 0:
        ...
        reporte.append("\nRECOMENDACIÓN: **ELIMINAR** estas variables (no aportan información). Ver 'variables_siempre_constantes.csv'.")
    else:
    ...
    reporte.append(f"\n📊 RESUMEN:")
    reporte.append(f"  - Variables a ELIMINAR (missings altos + constantes): {len(vars_eliminar)}")
    reporte.append(f"  - Variables con drift ALTO: {len(vars_drift_alto_df)}")
    
    reporte.append("\n✅ ACCIONES SUGERIDAS:")
    reporte.append("1. **Limpieza de Features:** Eliminar las {len(vars_eliminar)} variables identificadas por missings/constancia.")
    reporte.append("2. **Monitoreo de Drift:** Investigar las variables con PSI > {UMBRAL_DRIFT_PSI} y su impacto en el modelo.")
...
def main():
    ...
    print(f"Mes de análisis de Drift: {MES_FINAL} vs Histórico")
    print("="*80)
    ...
    print("\nCargando dataset...")
    try:
        # Usamos os.path.expanduser para manejar rutas como '~/'
        df = pd.read_csv(os.path.expanduser(DATASET_PATH), compression='gzip')
        print(f"✓ Dataset cargado: {df.shape}")
    except FileNotFoundError:
        print(f"\n❌ ERROR: Archivo no encontrado en la ruta: {DATASET_PATH}")
        sys.exit(1)
    except Exception as e:
    ...
    # **NOTA:** La función ya compara 202108 vs TODOS los anteriores
    df_psi, vars_drift_alto_df = analizar_data_drift(df) 
    ...
    generar_reporte_final(
        ...
        vars_drift_alto_df, # Pasamos el DataFrame con los resultados de PSI
        vars_baja_varianza,
        vars_siempre_constantes
    )
    ...
    print("  📊 03_data_drift_psi.png (Gráfico de Drift)")
